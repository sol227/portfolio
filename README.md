...
- html/css - lush 상세페이지 작업
- html/css - 프로젝트 상세페이지가 프로젝트에 어울리도록 포인트 색을 각 프로젝트의 메인컬러로 수정
- html/css - 프로젝트 상세페이지의 본문 크기가 너무 작아 가독성이 좋지 않아 크기 키움
- css - 상세페이지에 공통적으로 들어가는 스타일은 코드의 반복을 줄이기 위해 common.css를 따로 만들어서 링크를 연결해서 사용하도록 함
- m.funch/lush 상세페이지 메인 이미지를 비디오로 변경
- 문제해결과 리뷰 부분이 가독성이 떨어져 line height와 padding으로 여백 수정
